...
- `roms_cice.html` — "coming soon" page for the ROMS-CICE coupled sea-ice model (under development), with a cartoon illustration.
...
- `assets/js/bathymetry-3d.js`, `data/bathymetry_3d.json` — the Home page's
  ...
  slab" of the model domain: flat top and real bathymetric relief on the
  bottom face both coloured by depth (same yellow-to-purple scale as the 2-D
  bathymetry map on `about.html`), land, coastline and side walls. Everything
  is drawn as explicit triangle meshes (each vertex carries its own lon, lat
  and depth) rather than Plotly's 2-D-array `surface` trace, which paired
  depths with the wrong positions on this curvilinear grid. The JSON is
  generated by `scripts/export_bathymetry_3d.py`. (`assets/img/overview/bathymetry_3d.png`
